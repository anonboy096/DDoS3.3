...
def approve_or_disapprove_user(message):
    ...
    if action == '/approve':
        if plan == 1:  # Attack ++ Plans Plan 🧡
            if users_collection.count_documents({"plan": 1}) >= 99:
                bot.send_message(chat_id, "*Approval failed: Attack ++ Plans Plan 🧡 limit reached (99 users).*", parse_mode='Markdown')
                return
        elif plan == 2:  # Attack ++ Plans++ Plan 💥
            if users_collection.count_documents({"plan": 2}) >= 499:
                bot.send_message(chat_id, "*Approval failed: Attack ++ Plans++ Plan 💥 limit reached (499 users).*", parse_mode='Markdown')
                return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "Your Attack ++ Plans Plan 🧡 is currently not available due to limit reached.")
            return
...
def attack_command(message):
    ...
    try:
        ...
        if user_data['plan'] == 1 and users_collection.count_documents({"plan": 1}) > 99:
            bot.send_message(chat_id, "*Your Attack ++ Plans Plan 🧡 is currently not available due to limit reached.*", parse_mode='Markdown')
            return
...
def send_welcome(message):
    ...
    # Create buttons
    btn1 = KeyboardButton("Attack ++ Plans Plan 🧡")
    btn2 = KeyboardButton("Attack ++ Plans++ Plan 💥")
...
def handle_message(message):
    if message.text == "Attack ++ Plans Plan 🧡":
        bot.reply_to(message, "*Attack ++ Plans Plan selected*", parse_mode='Markdown')
    elif message.text == "Attack ++ Plans++ Plan 💥":
        bot.reply_to(message, "*Attack ++ Plans++ Plan selected*", parse_mode='Markdown')
        attack_command(message)
    elif message.text == "Canary Download✔️":
        bot.send_message(message.chat.id, "*Please use the following link for Canary Download: https://t.me/PrivateFileTg/2183*", parse_mode='Markdown')
